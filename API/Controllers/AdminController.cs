...
public class AdminController(IUnitOfWork unit, IPaymentService paymentService): BaseApiController
{
    ...
    public async Task<ActionResult<OrderDto>> GetOrderById(int id)
    {
        ...
        if (order == null) return BadRequest("Order not found with the provided ID");
        return order.ToDto();
    }
    ...
    public async Task<ActionResult<OrderDto>> RefundOrder(int id)
    {
        ...
        {
            return BadRequest("Payment has not been processed for this order.");
        }

        var result = await paymentService.RefundPayment(order.PaymentIntentId);
        if (result != "succeeded") return BadRequest("Refund failed. Please try again later.");
